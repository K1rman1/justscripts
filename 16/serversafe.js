...
app.get('/documents/:id', requireAuth, (req, res) => {
    ...
    db.get(`SELECT * FROM documents WHERE id = ? AND owner_id = ?`, [docId, userId], (err, doc) => {
        if (!doc) {
            return res.send(`
                Доступ запрещён или документ не найден
                <br><br>
                <a href="/">Назад</a>
            `);
        }
        res.send(`
            <h1>${doc.title}</h1>
            <p>${doc.content}</p>
            <p><small>Вы владелец этого документа</small></p>
            <a href="/">Назад</a>
        `);
    });
});
