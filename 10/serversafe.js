...
const SECRET_KEY = 'skjgklfdgjalkdhhladfkghjkldafgkajdhklfgfgd';
...
function createToken(user) {
    ...
    const signature = crypto.createHmac('sha256', SECRET_KEY).update(dataB64).digest('hex');
...
function verifyToken(token) {
    try {
        ...
        const expectedSignature = crypto.createHmac('sha256', SECRET_KEY).update(dataB64).digest('hex');
...
app.post('/login', (req, res) => {
    ...
    if (remember === 'on') {
        ...
        res.cookie('remember', token, { maxAge: 86400000, httpOnly: true });
    }
