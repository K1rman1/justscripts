...
app.get('/', (req, res) => {
    ...
    db.all(`SELECT * FROM products`, [], (err, products) => {
        ...
        res.send(`
            <h1>Интернет-магазин (Безопасный)</h1>
            <h2>Товары:</h2>
            ${productsHtml}
            <h2>Корзина:</h2>
            ...
            <p style="color:green">Цена берётся из БД</p>
        `);
    });
});
...
app.post('/cart/add', (req, res) => {
    ...
    db.get(`SELECT name, price FROM products WHERE id = ?`, [product_id], (err, product) => {
        if (!product) {
            return res.status(400).send('Товар не найден <a href="/">Назад</a>');
        }
    ...
    db.all(`SELECT id, price FROM products`, [], (err, products) => {
        ...
        req.session.cart.forEach(function(item) {
            ...
            if (dbPrice === undefined) {
                return res.status(400).send('Товар больше не доступен');
            }
